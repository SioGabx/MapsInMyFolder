...
namespace MapsInMyFolder
{
    public partial class MainPage : System.Windows.Controls.Page
    {
        ...
        public List<string> Search(string search)
        {
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    {
                        Search_result_list.search_list.Clear();
        ...
        private void Map_searchbar_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            Map_searchbar_timer.Elapsed += new ElapsedEventHandler(Map_searchbar_Timer_Elapsed_StartSearch);
        ...
        async void SearchStart(Boolean selectfirst = false)
        {
            ...
            await App.Current.Dispatcher.BeginInvoke(DispatcherPriority.Normal, (SendOrPostCallback)async delegate
            {
                ...
                if (text != "" && text != last_search)
                {
                    ...
                    Task search_task = Task.Run(() =>
                    {
                        list_of_search_result_from_task = Search(text);
                    });
                    await search_task;
    ...
    static public class Search_result_list
    {
        public static List<Dictionary<int, Search_result_engine_class>> search_list = new List<Dictionary<int, Search_result_engine_class>>() { };
    }
    ...
    public class Search_result_engine_class
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Performance", "CA1822:Marquer les membres comme étant static", Justification = "Used outside")]
        public int Add(Search_result_engine_class engine, int id)
        {
            int number_of_engine_in_list = Search_result_list.search_list.Count + 1;
            Dictionary<int, Search_result_engine_class> temp_dictionnary = new Dictionary<int, Search_result_engine_class>
            {
                { id, engine }
            };
            Search_result_list.search_list.Add(temp_dictionnary);
        ...
        public List<Search_result_engine_class> GetEngineList()
        {
            ...
            foreach (Dictionary<int, Search_result_engine_class> search_dictionnary in Search_result_list.search_list)
            {
